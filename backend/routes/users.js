...
router.post('/', verifyToken, authorizeRoles('admin'), async (req, res) => {
  try {
    ...
    const subject = 'Your WorkZen HRMS account details';
    const plain = `Hello ${name},\n\nYour WorkZen HRMS account has been created.\n\nEmail: ${email}\nPassword: ${autoGeneratedPassword}\n\nLogin here: ${appUrl}/login\n\nPlease change your password after logging in.`;
    const html = `
      <p>Hello ${name},</p>
      <p>Your WorkZen HRMS account has been created.</p>
      <p><strong>Email:</strong> ${email}<br/>
      <strong>Password:</strong> ${autoGeneratedPassword}</p>
      <p><a href="${appUrl}/login" target="_blank">Login to WorkZen HRMS</a></p>
...
router.post('/profile/image', verifyToken, (req, res) => {
  // Manually invoke multer to intercept errors
  upload.single('image')(req, res, async (err) => {
    try {
      if (err) {
        const msg = err.message || 'Invalid image upload';
      ...
      // Update user profile image and mime type
      await pool.query(
        'UPDATE users SET profile_image = $1, profile_image_mime = $2 WHERE id = $3',
        [req.file.buffer, req.file.mimetype || 'image/jpeg', req.user.id]
      );
...
router.get('/profile/image/:id', verifyToken, async (req, res) => {
  try {
    ...
    // All authenticated users can fetch profile images
    ...
    // Prevent caching so the latest image is always fetched
